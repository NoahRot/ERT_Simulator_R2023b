...
if boolean1 == 1 
    ...
    for b = azimuth_i
        j = j +1;
        [c,~,~, ~, ~, ~] = landing_tool(n_sim,-1, b/180*pi, Rocket_0, SimOutputs,name_of_environnment); 
        azi(j) = azi(j) + c;
    end
...
[min_area,r_ellipse,r_ellipse1, X0, Y0, data] = landing_tool(n_sim,-1,azimuth_i/180*pi, Rocket_0, SimOutputs,name_of_environnment);
